  ...
  AFRAME.registerSystem('mindar-image-system', {
    ...
    setup: function ({
      ...
      ;(this.imageTargetSrc = t),
        ...
        (this.filterMinCF = l),
        (this.filterBeta = d),
        (this.missTolerance = o),
        (this.warmupTolerance = h),
        (this.showStats = s),
        (this.ui = new e({ uiLoading: a, uiScanning: n, uiError: r }))
    },
    ...
    _startVideo: function () {
      ...
      navigator.mediaDevices
        ...
        .catch((t) => {
          console.log('getUserMedia error', t),
            this.el.emit('arError', { error: 'VIDEO_FAIL' })
        })
    },
          ...
          onUpdate: (t) => {
            if ('processDone' === t.type)
              ...
              for (let t = 0; t < this.anchorEntities.length; t++)
                this.anchorEntities[t].targetIndex === e &&
                  (this.anchorEntities[t].el.updateWorldMatrix(i),
                  i && this.ui.hideScanning())
            }
          },
        })),
        this._resize(),
        window.addEventListener('resize', this._resize.bind(this))
      const { dimensions: i } = await this.controller.addImageTargets(
        this.imageTargetSrc
      )
      for (let t = 0; t < this.anchorEntities.length; t++) {
        const { el: e, targetIndex: s } = this.anchorEntities[t]
    ...
    _resize: function () {
      ...
      a > e.clientWidth / e.clientHeight
        ? ((s = e.clientHeight), (i = s * a))
        : ((i = e.clientWidth), (s = i / a))
      ...
      ;(d.fov = r),
        ...
        (this.video.style.top = -(s - e.clientHeight) / 2 + 'px'),
        (this.video.style.left = -(i - e.clientWidth) / 2 + 'px'),
        (this.video.style.width = i + 'px'),
    ...
    AFRAME.registerComponent('mindar-image', {
      ...
      init: function () {
        ...
        t.setup({
          ...
          filterMinCF:
            -1 === this.data.filterMinCF ? null : this.data.filterMinCF,
          filterBeta: -1 === this.data.filterBeta ? null : this.data.filterBeta,
          missTolerance:
            -1 === this.data.missTolerance ? null : this.data.missTolerance,
          warmupTolerance:
            -1 === this.data.warmupTolerance ? null : this.data.warmupTolerance,
    ...
    AFRAME.registerComponent('mindar-image-target', {
      dependencies: ['mindar-image-system'],
      schema: { targetIndex: { type: 'number' } },
      postMatrix: null,
      init: function () {
        this.el.sceneEl.systems['mindar-image-system'].registerAnchor(
          this,
      ...
      setupMarker([t, e]) {
        ...
        ;(i.x = t / 2),
          ...
          this.postMatrix.compose(i, s, a)
      },
      updateWorldMatrix(t) {
        if (
          (this.el.object3D.visible || null === t
          ...
          ;(e.elements = t),
            e.multiply(this.postMatrix),
            (this.el.object3D.matrix = e)
        }
      },
    })
})()
